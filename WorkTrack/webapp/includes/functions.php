...
function formatDateNL(string $date, bool $withDay = true): string {
    static $months = ['', 'januari', 'februari', 'maart', 'april', 'mei', 'juni',
                      'juli', 'augustus', 'september', 'oktober', 'november', 'december'];
    static $days   = ['zondag', 'maandag', 'dinsdag', 'woensdag', 'donderdag', 'vrijdag', 'zaterdag'];
    $ts = strtotime($date);
    $d  = (int) date('j', $ts);
...
function getWorkoutWithExercises(int $workoutId): ?array {
    ...
    $stmt = $pdo->prepare('
        SELECT we.id, we.order_index, we.notes AS we_notes,
               e.id AS exercise_id, COALESCE(we.custom_name, e.name_nl) AS name,
